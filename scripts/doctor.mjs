...
if (!fs.existsSync(path.join(repoRoot, "tutor"))) {
  ...
  const progress = readJson("tutor/progress.json");
  const quizBank = readJson("tutor/quiz-bank.json");
  ...
  if (journalExists) {
    ...
    while ((m = re.exec(text)) !== null) {
      if (!newestJournal || m[1] > newestJournal) newestJournal = m[1];
    }
  }
  if (!quizBank.ok) {
